...
- **Hand test:** `no` by default, with a clause naming the agent check that covers the change. Triage first writes each visible change (layout, wording, keys, timing, a safety prompt) as an acceptance criterion an agent can run: a `scripts/gate snapshot` state at a stated size with its expected text, a PTY check in an acceptance script, a rendered-buffer test, an exit status and its stderr text. `yes` is for the remainder only a hand can judge in the real terminal (colour, motion, how a key feels, whether a layout looks right), and carries numbered "do X, see Y" steps for that remainder alone. `yes` hands the close to the owner (`docs/agents/gate.md` § Feature tier); `no` lets a green PR land without asking.
