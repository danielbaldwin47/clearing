...
with open(ROOT/'.runtime/referee-state.lock','w') as lock:
 ...
 if s.get('frozen') or (ROOT/'.runtime/FROZEN').exists() or now>=deadline:raise SystemExit('Hard deadline reached: no verdict can be submitted')
 ...
 if (ROOT/'.runtime/FROZEN').exists() or dt.datetime.now(dt.timezone.utc)>=deadline:raise SystemExit('Hard deadline reached before publication')
