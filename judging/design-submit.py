...
with open(ROOT/'.runtime/design-state.lock','w') as lock:
 ...
 if s.get('frozen') or (ROOT/'.runtime/DESIGN-FROZEN').exists() or now>=deadline:raise SystemExit('Hard deadline reached: no verdict can be submitted')
 ...
 if (ROOT/'.runtime/DESIGN-FROZEN').exists() or dt.datetime.now(dt.timezone.utc)>=deadline:raise SystemExit('Hard deadline reached before publication')
