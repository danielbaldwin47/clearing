...
def clean(path,out,content_crop=None):
 ...
 # Shortened machine-path prefixes can still expose source provenance.
 transcript=path.with_suffix('.txt')
 if transcript.exists():
  import re
  for y,line in enumerate(transcript.read_text().splitlines()):
   for match in re.finditer(r'[^\s│┃]*(?:/fixtures/)([^\s│┃]*)',line):
    if '/home/diggle/Work/tui-disk/fixtures/' in match.group(0):continue
    start,end=match.span();tailstart=match.start(1);row=im.crop((tailstart*9,y*20,end*9,(y+1)*20));bg=im.getpixel((max(0,start*9-1),min(im.height-1,y*20+1)));im.paste(bg,(start*9,y*20,end*9,(y+1)*20));im.paste(row,(start*9,y*20))
 # Remove only identity tokens; preserve task labels and application controls.
 transcript=path.with_suffix('.txt')
 if transcript.exists():
  import re
  from PIL import ImageDraw
  for y,line in enumerate(transcript.read_text().splitlines()):
   for token in ('superfile','btop','DISK ALLOCATION'):
    for match in re.finditer(re.escape(token),line):
     x=match.start()*9;bg=im.getpixel((max(0,x-2),min(im.height-1,y*20+2)));im.paste(bg,(x,y*20,min(im.width,match.end()*9),(y+1)*20))
...
def fingerprint(piece='whole'):
 ...
 return h.hexdigest()
def create(a):
 if read().get('stop_after_current_round'):raise SystemExit('User stopped new rounds')
 ...
 if not a.piece.startswith('calibration'):
 ...
 for i,token in enumerate(tokens):
  pid='p'+token;dest=ROOT/'judging/design-blind'/pid;dest.mkdir(parents=True);current=order if i==0 else order[::-1];mapping={k:v for k,v in zip('AB',current)}
  ...
  brief='Two interface entries are provided as A.png and B.png. Inspect only these images and this brief. Do not inspect other files, browse, infer authorship, or identify products.\n\n'+a.brief+'\n\nWhich of these two visual systems would you rather spend time using for dense terminal data? Judge visual hierarchy, legibility, color, spacing, selection clarity, and composed detail. Do not penalize differences in task domain or available features. Choose exactly A or B. Cite concrete visible evidence in each entry. Give the single biggest reason the loser lost. Be harsh; praise is not useful. Return JSON with choice, evidence_A, evidence_B, and loser_reason.\n'
...
def sequence(a):
 ...
 source_fp=fingerprint(a.piece);provenance=[]
 ...
 pid='p'+secrets.token_hex(8);dest=ROOT/'judging/design-blind'/pid;dest.mkdir(parents=True);order=['candidate','bar'];secrets.SystemRandom().shuffle(order);mapping=dict(zip('AB',order))
 ...
 brief='Two coherent terminal interface entries are provided as A.png and B.png contact sheets. Inspect all full-resolution panels: '+', '.join(f'{l}-{i+1}.png' for l in 'AB' for i in range(len(candidates)))+'. Both entries have the same state sequence: '+', '.join(f'{i+1}={label}' for i,label in enumerate(a.states))+'. Each entry comes from one interface with one coherent visual system.\n\n'+a.brief+'\n\nWhich of these two visual systems would you rather spend time using for dense terminal data? Judge hierarchy, typography, color, spacing, readable selection and composed state transitions. Do not penalize task-domain or feature differences; do not infer authorship or product identities. Choose exactly A or B. Cite concrete visible evidence in each entry and give the single biggest reason the loser lost. Praise is not useful. Return JSON with choice, evidence_A, evidence_B and loser_reason.\n'
 (dest/'brief.txt').write_text(brief);key={'id':pid,'piece':a.piece,'mapping':mapping,'labels':{'candidate':a.candidate_label,'bar':a.bar_label},'sources':{'candidate':[str(p) for p in candidates],'bar':[str(p) for p in bars]},'source_fingerprint':source_fp,'group':pid,'orientation':0,'created_at':now(),'audit':a.audit};(PRIVATE/(pid+'.enc')).write_bytes(crypt(json.dumps(key).encode()))
 s=read();blocked=[k for k,p in s['pieces'].items() if k!='whole' and not k.startswith('calibration') and p['status']!='won'] if a.piece=='whole' else [];s['rounds'].append({'id':pid,'piece':a.piece,'packet':'/judging/design-blind/'+pid,'status':'awaiting verdict','created_at':now(),'source_fingerprint':source_fp,'capture_binary_sha256':next(iter(hashes)),'frames':a.states,'blocked_by':blocked});s['pieces'][a.piece]['rounds'].append(pid);s['phase']=a.piece+' comparison prepared'+(' but judging blocked by '+', '.join(blocked) if blocked else '');write(s);print(json.dumps({'packet':str(dest),'id':pid,'blocked_by':blocked,'source_fingerprint':source_fp}))
def record(a):
 ...
 result=key['mapping'][choice];
 if a.count and not key['piece'].startswith('calibration') and key['source_fingerprint']!=fingerprint(key['piece']):raise SystemExit('Stale source cannot count')
 ...
 piece=s['pieces'][key['piece']];desired='bar' if key['piece'].startswith('calibration') else 'candidate'
...
def validate(a):
 ...
 if not r['piece'].startswith('calibration') and r.get('source_fingerprint')!=fingerprint(r['piece']):raise SystemExit('Source changed; cannot validate stale verdict')
 desired='bar' if r['piece'].startswith('calibration') else 'candidate'
...
def finish(a):
 ...
 for name,piece in s['pieces'].items():
  ...
  if not name.startswith('calibration') and piece.get('source_fingerprint')!=fingerprint(name):raise SystemExit('Cannot finish: '+name+' source changed')
 actual=hashlib.sha256((ROOT/'target/release/tui-disk').read_bytes()).hexdigest()
 s['completed']=True;s['completed_at']=now();s['release_sha256']=actual;s['phase']='All six design pieces won after independent referee audits';s['events'].append({'time':now(),'message':'Design gauntlet completed by winning before deadline. No performance claim is made for this release.'});write(s);print(json.dumps({'complete':True,'release_sha256':actual,'completed_at':s['completed_at']}))
def check_stale(a):
 s=read();changed=[]
 for name,piece in s['pieces'].items():
  if name.startswith('calibration') or not piece.get('valid_wins'):continue
  if piece.get('source_fingerprint')!=fingerprint(name) or (name=='performance' and piece.get('binary_sha256')!=hashlib.sha256((ROOT/'target/release/tui-disk').read_bytes()).hexdigest()):
...
w=sub.add_parser('sequence');w.add_argument('--piece',default='whole');w.add_argument('--brief',required=True);w.add_argument('--candidate-label',default='Current design candidate');w.add_argument('--bar-label',default='Saved polished terminal file manager');w.add_argument('--candidate',nargs='+',required=True);w.add_argument('--bar',nargs='+',required=True);w.add_argument('--states',nargs='+',required=True);w.add_argument('--audit',required=True)
