...
9. **Land.** Hand test `no`: `gh pr merge <PR> --merge --delete-branch`, pull `main`, `scripts/gate check` on it, remove the worktree, and a closing comment on the ticket; a green PR lands without asking. Hand test `yes`: the Hand test comment goes to the owner and the PR waits (`docs/agents/gate.md` § Feature tier).
