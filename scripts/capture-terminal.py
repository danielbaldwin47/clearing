...
def guard():
 if pathlib.Path('.runtime/FROZEN').exists() or datetime.datetime.now(datetime.timezone.utc) >= datetime.datetime(2026,9,20,0,46,22,tzinfo=datetime.timezone.utc):
...
try:
 ...
 procs.append(subprocess.Popen(cmd,env=env,stdout=log,stderr=log));time.sleep(a.settle)
